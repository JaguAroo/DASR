...
#     DASR.load_state_dict(torch.load(dir + '/model/model_' + str(args.resume) + '.pt'),
#                          strict=False)
#     DASR.eval()
#     print(args.img_dir)
...
#     # save sr results
#     img_name = args.img_dir.split('.jpg')[0].split('/')[-1]
#     sr = np.array(sr.squeeze(0).permute(1, 2, 0).data.cpu())
#     sr = sr[:, :, [2, 1, 0]]
#     cv2.imwrite(save_dir + '/' + img_name + '_' + args.scale + '_sr.png', sr)
